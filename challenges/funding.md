...
The Transparent Approach to Costing (TRAC) is a methodology used by UK higher
education institutions to calculate the Full Economic Cost (FEC) of research
projects. The FEC includes Directly Incurred (DI), Directly Allocated (DA), and
Overheads. Using the TRAC approach means that a day rate is calculated for each
RSE, which is then used to calculate the cost of the RSE's time on a project. This
rate is based on FEC and is used to recover costs from projects.

**NOTE**: Applications for TRAC are currently blocked
...
or by using a rate based on salary alone.
...
- The day rate may not encorporate salary inflation either due to national inflation
  or changes in spinal point.
- When setting a day rate without a specific RSE in mind you need to consider which
  spinal point to use. If using the spinal point in the middle of the grade this
  may average out but will depend on the RSEs in the group.
- A different rate may be needed for each grade of RSE in the group.
- May need to include percent of senior time in the rate for junior RSEs to cover
  mentoring, training and project management.

Also see [project_management#Timesheets](./project_management.md#Timesheets)

## Impact on the researchers

The funding model will impact how researchers are able to access RSEs. We can either
be free at the point of use or we need to cost recover from projects.
...
- It can be difficult to trace funding from the funder through the institution and
  back to RSE team.
